...
router.get("/getdetails", verifyToken, async (req, res) => {
  try {
    ...
    const response = await fetch(
      `https://api.uniblock.dev/direct/v1/CoinGecko/coins/${coinId}`,
      {
        headers: {
    ...
    if (response.ok) {
      ...
      res
        .status(400)
        .json({ error: "Error fetching coin data from Uniblock API" });
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (existingCoin) {
        ...
        return res
          .status(200)
          .json({ message: "Coin updated in the database", coin: coinDetails });
      } else {
        ...
        res
          .status(201)
          .json({ message: "Coin added to database", coin: newCoin });
      }
    } else {
      res
        .status(400)
